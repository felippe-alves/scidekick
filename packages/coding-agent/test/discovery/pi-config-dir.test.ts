...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath uses PI_CONFIG_DIR for native userAgent", () => {
		process.env.PI_CONFIG_DIR = ".config/sk";
		...
		expect(result).toBe(path.join(ctx.home, ".config/sk/agent", "commands"));
	});

	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		process.env.PI_CONFIG_DIR = ".config/sk";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/sk", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".sk", level: "user" });
	});
});
